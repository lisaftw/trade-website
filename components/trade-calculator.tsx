    ...
    return (
      <div className="space-y-4">
        <div className="text-center">
          <h1 className="text-2xl font-bold tracking-wide text-white md:text-3xl">Trade Calculator</h1>
          <p className="mt-1 text-xs text-gray-400">Select a game to start trading</p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          {(["MM2", "SAB", "GAG", "Adopt Me"] as const).map((g) => (
            <button
              key={g}
              onClick={() => setGame(g)}
              className="rounded-lg border border-gray-700 bg-gray-800/50 p-4 text-left transition-all hover:border-gray-600 hover:bg-gray-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
            >
              <p className="text-base font-semibold text-white">{g}</p>
              <p className="mt-0.5 text-[10px] text-gray-400">Load items for {g}</p>
            </button>
          ))}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-black py-6">
      <div className="mx-auto max-w-6xl px-4">
        <div className="mb-3 text-center">
          <div className="inline-flex items-center rounded-full border border-gray-700 bg-gray-900/80 px-3 py-1.5 text-xs text-white">
            ...
            <button
              ...
              className="ml-2 rounded-full border border-gray-600 px-2 py-0.5 text-[10px] transition-colors hover:bg-gray-700"
            >
              Change
            </button>
          </div>
        ...
        <div className="relative rounded-2xl border-2 border-gray-700/50 bg-gradient-to-b from-gray-900/40 to-black/60 p-4 backdrop-blur-sm md:p-6">
          <div className="mb-4 flex items-center justify-center">
            <Image src="/trader-logo.png" alt="TRADER" width={200} height={60} className="h-auto w-40" priority />
          </div>
  ...
  return (
    <div className="flex flex-col">
      <div className="mb-3 flex justify-center">
        <div className="rounded-full border-2 border-gray-600/80 bg-black/90 px-6 py-1.5 shadow-lg">
          <span className="text-base font-bold tracking-wide text-white">{title}</span>
        </div>
      </div>
            ...
            {index === items.length && !item ? (
              ...
              <div className="group relative h-full w-full p-1.5">
                ...
                <button
                  ...
                  className="absolute right-1.5 top-1.5 rounded-full bg-red-500/90 p-1 opacity-0 transition-opacity hover:bg-red-600 group-hover:opacity-100"
                >
                  <X className="h-3 w-3 text-white" />
                </button>
                <div className="absolute bottom-0 left-0 right-0 rounded-b-lg bg-black/90 p-1.5 text-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4">
          <div className="w-full max-w-2xl rounded-xl border-2 border-gray-700 bg-gray-900 p-4 shadow-2xl">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-lg font-bold text-white">Add Item to {title}</h3>
              <button
                onClick={onClose}
            ...
            <div className="max-h-80 space-y-1.5 overflow-y-auto">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                  <div className="mb-2 h-6 w-6 animate-spin rounded-full border-4 border-gray-700 border-t-white" />
                  <p className="text-sm">Loading {selectedGame} items...</p>
                </div>
              ) : error ? (
                <div className="py-10 text-center text-sm text-red-400">{error}</div>
              ) : displayedItems.length === 0 && searchQuery ? (
                <div className="py-10 text-center text-sm text-gray-400">No items found matching "{searchQuery}"</div>
              ) : displayedItems.length === 0 ? (
                <div className="py-10 text-center text-sm text-gray-400">No items available for {selectedGame}</div>
              ) : (
                <>
                  ...
                  {displayedItems.map((item) => (
                    <button
                      ...
                      className="flex w-full items-center gap-3 rounded-lg border border-gray-700 bg-gray-800 p-2.5 text-left transition-all hover:border-gray-600 hover:bg-gray-750"
                    >
                      <Image
                        src={item.imageUrl || "/placeholder.svg"}
                        alt={item.name}
